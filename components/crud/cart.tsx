...
export const Cart = () => {
    ...
    const updateQuantity = (
        ...
        if (validQuantity !== quantity) {
            toast({
                ...
                description: `Maximum available quantity for ${name} is ${maxQuantity}`,
                variant: "destructive",
            });
        }
        ...
        dispatch({
            ...
            payload: { id, quantity: validQuantity },
        });
    };
    ...
    const handleSubmitRequest = async () => {
        try {
            ...
            // Check for stale data before submission
            const itemIds = state.items.map((item) => item.id).join(",");
            ...
            const staleItems = state.items.filter((cartItem) => {
                const dbItem = currentItems.find(
                    (item: any) => item.id === cartItem.id
                );
                return !dbItem || cartItem.quantity > dbItem.quantity;
            });

            if (staleItems.length > 0) {
                // Sync cart with current data
                ...
                toast({
                    title: "Cart Updated",
                    description:
                        "Some items in your cart were updated due to stock changes",
                    variant: "destructive",
                });
