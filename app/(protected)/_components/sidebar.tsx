...
import { Menu, Boxes, LogOut, ShieldCheck, Building2, Users } from "lucide-react";
        ...
        >
            ...
            {isApproved && (
              <div className="space-y-2 py-4">
                {navItems.map((item) => {
                  ...
                  return (
                    <Link key={item.href} href={item.href} className="group" title={item.title}>
                      <span
                        className={cn(
                          "flex items-center py-3  rounded-lg",

                          isActive(item.href) ? "bg-[#ff9e9e]" : "group-hover:text-[#ff9e9e] ",
                          isExpanded ? "justify-start mx-6 px-4" : "justify-center mx-2 px-1"
                        )}
                      >
                        <item.icon
                          className={cn(
                        ...
                        {isExpanded && (
                          <span
                            className={cn("font-medium", isActive(item.href) && "text-[#050617]")}
                          >
                            {item.title}
                          </span>
                        )}
          ...
          <div className="mb-5 flex flex-col gap-2 border-t border-[#f5d4d4]/10 backdrop-blur-sm">
                ...
                <AccordionItem value="profile" className="border-b-0">
                  <AccordionTrigger
                    className={`flex items-center mt-4 ${
                      isExpanded ? "mx-6 justify-between gap-2" : "mx-2 justify-center"
                    }`}
                  >
                    <Image
                      src={
                  ...
                  <AccordionContent>
                    <div className="flex flex-col gap-2 border-t border-[#f5d4d4]/10 backdrop-blur-sm py-2 mt-4 mx-2">
                      ...
                      <span
                        className="flex gap-2 justify-left items-center ms-2"
                        title={divisionMap[session?.user?.division ?? ""]}
                      >
                        <Building2 className="w-4 h-4" />
                        <span className="flex flex-col">
                          <span className="text-[11px]">Division</span>
                          <span className="text-xs truncate w-[192px]">
                            {divisionMap[session?.user?.division ?? ""]}
                          </span>
                        </span>
                      </span>
                      <span
                        className="flex gap-2 justify-left items-center ms-2"
                        title={sectionMap[session?.user?.section ?? ""]}
                      >
            ...
            >
              ...
              >
                {isExpanded && "Sign Out"}
                <LogOut className={`${isExpanded && "mx-2"} w-4 h-4`} />
              </Button>
            </div>
          </div>
        </div>
    ...
    <div className="h-full flex flex-col bg-[#7b1113] text-[#f5d4d4]">
      ...
      <ScrollArea className="flex-1">
        {isApproved && (
          <div className="space-y-2 py-4">
            {navItems.map((item) => {
              if (item.roles && !item.roles.includes(session?.user?.role as string)) return null;
              ...
              return (
                <Link key={item.href} href={item.href} className="group">
                  ...
                  >
                    ...
                    />
                    <span className={cn("font-medium", isActive(item.href) && "text-[#050617]")}>
                      {item.title}
                    </span>
                  </span>
                </Link>
